...
const DashboardB = () => {
  ...
  const partnerLabel = mode === "family" ? "PARENT/TEEN" : "PARTNER A";
  ...
  return (
    <div className="min-h-screen pt-16" style={{ backgroundColor: "hsl(25 15% 10%)" }}>
      {/* Header */}
        ...
        <div className="container mx-auto px-6 py-6 flex items-center justify-between">
          <div>
            ...
            <h1 className="heading-display text-2xl lg:text-4xl text-primary-foreground">
              YOUR SPACE, {setup.partnerB.name.toUpperCase()}
            </h1>
          </div>
          ...
          <div className="container mx-auto px-6 pb-6">
            <div className="border-2 border-silver/20 p-4 max-w-sm">
              ...
              <div className="flex gap-2">
                <button onClick={() => setMode("partnership")} className={`heading-label text-[9px] px-4 py-2 transition-colors ${mode === "partnership" ? "bg-primary-foreground text-primary" : "border border-silver/30 text-silver"}`}>
                  PARTNERSHIP
                </button>
                <button onClick={() => setMode("family")} className={`heading-label text-[9px] px-4 py-2 transition-colors ${mode === "family" ? "bg-primary-foreground text-primary" : "border border-silver/30 text-silver"}`}>
                  FAMILY
                </button>
              </div>
            </div>
      ...
      <div className="container mx-auto px-6 py-12 max-w-4xl space-y-12">
        ...
        <AnimatedSection>
          <span className="heading-label text-[10px] text-silver mb-4 block">SHARED FROM {setup.partnerA.name.toUpperCase()}</span>
          {sharedFromA.length === 0 ? (
            <div className="border-2 border-silver/10 p-6 text-center">
              <p className="font-body text-silver/50 text-sm">{setup.partnerA.name} hasn't shared any cards yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {sharedFromA.map((card) => (
                <div key={card.key} className="p-6 border-4" style={{ borderColor: "hsl(215 30% 30%)" }}>
        ...
        <AnimatedSection>
          <div className="text-center">
            {canEnterRoom ? (
              <Link to="/couple-room" className="heading-label text-sm bg-primary-foreground text-primary px-10 py-4 hover:bg-silver transition-colors inline-flex items-center gap-3">
                <Users size={16} /> ENTER SHARED SPACE
              </Link>
            ) : (
              <div className="border-2 border-silver/10 p-6">
                <p className="font-body text-silver/50 text-sm">
                  {sharedPaused.B ? "Shared features are paused." : "Both partners need to share cards to enter the couple room."}
                </p>
              </div>
            )}
          </div>
